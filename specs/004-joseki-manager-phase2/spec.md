...
### CD-004: SFEN 出力 Property

**決定**: Node モデルにデフォルトで手数ありの SFEN を出力する property を追加

**実装方針**:
- `Node.sfen`: 手数なし SFEN（データベース保存用、一意識別用）
- `Node.sfen_with_ply`: 手数あり SFEN を返す property（`{sfen} {ply}` 形式）
- デフォルトの `__str__` や表示用途では手数あり形式を使用

**理由**:
- 手数情報は棋譜表示や cshogi との連携で頻繁に必要
- property として提供することで、呼び出し側のコードがシンプルになる
- 手数なし形式は内部的なキーとして使用し、手数あり形式は外部インターフェース用

### CD-005: Edge の move_japanese フィールドの必須性

**決定**: 任意（NULLABLE）

**実装方針**:
- `Edge.move_japanese`: NULLABLE（任意）
- 日本語表記が必要な場合は `move_usi` から後で生成可能

**理由**:
- 日本語表記は表示用途のため、データ入力時に必須ではない
- `move_usi` から cshogi を使用して後から生成可能
- データ入力の簡略化と柔軟性を優先
