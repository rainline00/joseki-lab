...
- [x] T001 Create project directory structure per plan.md (`src/joseki/`, `src/trees/`, `src/labels/`, `src/exports/`, `src/core/`, `tests/`)
- [x] T002 Initialize uv project with `uv init` and configure pyproject.toml
...
- [x] T003 [US1] Add main dependencies to pyproject.toml (Django 5.0+, DRF 3.14+, django-filter, psycopg[binary], cshogi 0.8+, drf-spectacular) using `uv add`
- [x] T004 [US1] Add dev dependencies to pyproject.toml (pytest 8.0+, pytest-django, pytest-cov, factory-boy, black, ruff, mypy, django-stubs, djangorestframework-stubs) using `uv add --dev`
- [x] T005 [US1] Create `.env.example` with DATABASE_URL, SECRET_KEY, DEBUG, ALLOWED_HOSTS
- [x] T006 [P] [US1] Configure tool settings in pyproject.toml (black, ruff, mypy, pytest)
...
- [x] T007 [US2] Create `src/joseki/__init__.py`
- [x] T008 [US2] Create `src/joseki/settings.py` with Django 5.0+ configuration (REST_FRAMEWORK, database, installed apps)
- [x] T009 [US2] Create `src/joseki/urls.py` with API versioning (`/api/v1/`)
- [x] T010 [US2] Create `src/joseki/wsgi.py`
- [x] T011 [P] [US2] Create `src/manage.py` at src directory
- [x] T012 [P] [US2] Create `src/trees/__init__.py`
- [x] T013 [P] [US2] Create `src/labels/__init__.py`
- [x] T014 [P] [US2] Create `src/exports/__init__.py`
- [x] T015 [P] [US2] Create `src/core/__init__.py`
...
- [x] T016 [US3] Create `tests/conftest.py` with pytest-django configuration
- [x] T017 [US3] Create `tests/factories.py` with factory_boy base setup
- [x] T018 [P] [US3] Create `tests/__init__.py`
...
- [x] T019 Update `specs/003-joseki-manager-phase1/quickstart.md` with uv-based setup instructions
- [x] T020 Validate all completion criteria from Issue #17
...
- [x] すべてのディレクトリ構造が作成されている
- [x] `uv sync` が成功する（requirements.txt → uv に変更）
- [x] `uv run python src/manage.py check` が成功する
- [x] `uv run pytest --collect-only` が実行可能
